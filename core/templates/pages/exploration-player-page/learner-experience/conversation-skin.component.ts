...
export class ConversationSkinComponent {
  ...
  ngOnInit(): void {
    ...
    this.userService.getUserInfoAsync().then(async userInfo => {
      ...
      if (this.collectionId) {
        this.collectionSummary = this.conversationFlowService.fetchCollectionSummary(
          this.collectionId
        );
      }

      if (this.isLoggedIn) {
        this.completedChaptersCount = this.conversationFlowService.fetchCompletedChaptersCount();
      }
  ...
  submitAnswer(
    ...
    this.answerIsCorrect = currentEngineService.submitAnswer(
      ...
      (
        ...
        setTimeout(() => {
          this.explorationPlayerStateService.onOppiaFeedbackAvailable.emit();
          this.conversationFlowService.setActiveVoiceover(feedbackHtml, this.displayedCard);
  ...
  private giveFeedbackAndStayOnCurrentCard(
    ...
    if (refreshInteraction) {
      ...
      this.playerTranscriptService.updateLatestInteractionHtml(
        this.displayedCard.getInteractionHtml() + this.conversationFlowService.getRandomSuffix()
      );
    }
  ...
  showUpcomingCard(): void {
    ...
    if (
      ...
      this.conversationFlowService.returnToExplorationAfterConceptCard();
      return;
    }
